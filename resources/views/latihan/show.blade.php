...
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-bold text-gray-800" style="font-family:'Plus Jakarta Sans',sans-serif">{{ $package->name }}</h1>
    @if($package->type === 'free')
    <span class="bg-green-100 text-green-700 text-sm font-bold px-3 py-1 rounded-full">GRATIS</span>
    @else
    <span class="bg-yellow-100 text-yellow-700 text-sm font-bold px-3 py-1 rounded-full">PREMIUM</span>
    @endif
</div>
...
<div class="bg-yellow-50 border border-yellow-200 text-yellow-700 rounded-xl px-4 py-3 mb-6 text-sm flex items-center gap-3">
    <span>👑</span>
    <span>Paket ini hanya tersedia untuk pengguna Premium. <a href="{{ route('premium') }}" class="font-semibold text-yellow-800 hover:underline">Upgrade sekarang!</a></span>
</div>
<button class="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-xl transition text-sm" disabled>
    Mulai Latihan
</button>
@else
